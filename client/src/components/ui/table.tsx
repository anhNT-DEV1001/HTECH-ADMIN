...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto border border-border/60 rounded-lg"
    >
      <table
        data-slot="table"
...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn("[&_tr]:border-b bg-muted/40 [&_tr:first-child]:rounded-t-lg", className)}
      {...props}
    />
  )
}

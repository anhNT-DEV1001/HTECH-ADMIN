...
export default function ExhibitorModal({
  ...
  const handleApplyCrop = async () => {
    ...
    const croppedFileName = `${(data?.name || "exhibitor").replace(/\s+/g, "-").toLowerCase()}.jpg`;
    ...
    <>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex flex-col gap-3 rounded-lg border border-dashed p-4 md:flex-row md:items-center">
                <div className="flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-lg border bg-muted/30">
                  {imagePreview ? (
                    <img src={imagePreview} alt="Exhibitor preview" className="h-full w-full object-cover" />
                  ) : (
      ...
      <Dialog open={isCropModalOpen} onOpenChange={setIsCropModalOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Crop ảnh exhibitor</DialogTitle>
          </DialogHeader>
